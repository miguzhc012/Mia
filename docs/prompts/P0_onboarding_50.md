# MIA — AGENT ONBOARDING & PROJECT CONTEXT
...
Mas não deve possuir unilateralmente a autoridade sobre todo o estado persistente da Mia.
...
# 11. DEPENDÊNCIA, APEGO E SOLIDÃO — EXPERIMENTO EXPLÍCITO
...
### REQUISITO DE TELEMETRIA

Como esse é um experimento, a trajetória desses estados precisa ser observável externamente.
...
Devemos poder produzir séries temporais como:
...
Isso é essencial para avaliar empiricamente o comportamento.

Sem telemetria, não será possível responder:

"Até onde esse sistema chega?"

"Como a solidão realmente evolui?"

"Quais eventos mais influenciam o apego?"
...
Essas necessidades podem influenciar motivação.

---

# 17. DESEJOS

Desejo não deve ser sinônimo de task.

A Mia deve poder possuir:

* objetivos atribuídos;
* objetivos autoformados;
* desejos;
* curiosidades;
* impulsos de exploração;
* vontades de interação.

Um desejo pode existir sem ser imediatamente executado.
...
A Mia deve possuir crenças com confiança.
...
A ordem de autoridade precisa ser explícita.

Uma hierarquia inicial proposta é:

1. System Integrity / Security Boundary
2. Resource & Budget Authority
3. Permission / Policy Authority
4. Approved Architecture & Contracts
5. Runtime
6. Mia Adaptive State
7. LLM interpretation/reasoning
8. Agent suggestions

IMPORTANTE:

Esta hierarquia é uma PROPOSTA INICIAL.

O agente que estiver revisando este documento deve avaliar:

* se essa ordem é coerente;
* onde existem conflitos;
* quais autoridades precisam de formalização;
* quais decisões devem virar ADR.

Não tratar automaticamente esta lista como definitiva sem revisão arquitetural.

---

# 40. STATE AUTHORITIES — DECISÃO EM ABERTO

A arquitetura precisa separar quem interpreta uma experiência de quem realmente altera estado.

Exemplo:

LLM:
"Isso parece ter deixado Mia desconfortável."

State Authority:
avalia essa interpretação.

Mas ainda existe uma decisão arquitetural aberta:

A State Authority será:

A) predominantemente determinística/regra;

B) parcialmente probabilística;

C) utilizará LLM internamente;

D) híbrida?

Esta questão é explicitamente:

OPEN ARCHITECTURAL DECISION.

Não escrever no documento como se já estivesse resolvida.

O agente responsável pelo planejamento deve analisar:
...
LONG-TERM REQUIREMENT
Objetivo obrigatório futuro, não necessariamente implementado agora.
...
Essa classificação deve aparecer na documentação quando relevante.
...
O objetivo é que qualquer outro agente que leia o mesmo documento consiga chegar a uma compreensão compatível da Mia.